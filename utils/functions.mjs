...
export function selamat_datang(client, aa, ad, intro_grup, nama_grup){
    if(xt.group.name == undefined){
        const asd = aa.replace("@c.us", "")
        const au = ad.replace("@g.us", "")
        const ra = intro_grup.replace("{user}", `@`+`${asd}`)
        const re = ra.replace("{group}",`${nama_grup}`)
        client.sendMessage(ad, re, {
            mentions: [aa]
        })
    }
    else{
        ...
        const au = ad.replace("@g.us", "")
        const ra = intro_grup.replace("{user}", `@`+`${asd}`)
        const re = ra.replace("{group}",`${xt.group.name}`)
        client.sendMessage(ad, re, {
            mentions: [aa]
        })
    }
...
export function selamat_tinggal(client, aa, ad, outro_grup, nama_grup){
    if(xt.group.name == undefined){
        const asd = aa.replace("@c.us", "")
        const au = ad.replace("@g.us", "")
        const ra = outro_grup.replace("{user}", `@`+`${asd}`)
        const re = ra.replace("{group}",`${nama_grup}`)
        client.sendMessage(ad, re, {
            mentions: [aa]
        })
