                    ...
                    if(pesan_bot.length == 2){
                        ...
                        if(result == 'enable'){
                          ...
                          con.connect(function(err) {
                              if (err) throw err;
                              con.query("SELECT * FROM outro_grup WHERE id_grup = '"+xt.group.id+"'", function (err, result) {
                                if (err) throw err;
                                if(result.length === 0){
                                  var sql = "INSERT INTO outro_grup (id_grup, outro_grup, status) VALUES ('"+id_grup+"', '', 'enable')";
                                  ...
                                  var sql = "UPDATE outro_grup SET status = 'enable' WHERE id_grup = '"+id_grup+"';";
                                  con.query(sql, function (err, result) {
                                      if (err) throw err;
                                      msg.reply('Set Leave berhasil diaktifkan')
                                  });
                                }
                              });
                            });
                          ...
                          con.connect(function(err) {
                              ...
                              con.query("SELECT * FROM outro_grup WHERE id_grup = '"+xt.group.id+"'", function (err, result) {
                                if (err) throw err;
                                if(result.length === 0){
                                  var sql = "INSERT INTO outro_grup (id_grup, outro_grup, status) VALUES ('"+id_grup+"', '', 'disable')";
                                  con.query(sql, function (err, result) {
                                      if (err) throw err;
                                      msg.reply('Set Leave berhasil dinonaktifkan')
                                  });
                                }
                                else{
                                  var sql = "UPDATE outro_grup SET status = 'disable' WHERE id_grup = '"+id_grup+"';";
                                  con.query(sql, function (err, result) {
                                      if (err) throw err;
                                      msg.reply('Set Leave berhasil dinonaktifkan')
                                  });
